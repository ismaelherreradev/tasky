import { type AnyColumn, and, asc, desc, eq, exists, type SQL, sql } from "drizzle-orm"
...
export type QueryOptions<T extends Record<string, AnyColumn>> = PaginationOptions & SortOptions<T>
...
export const normalizePagination = (
  ...
  const limit = Math.min(MAX_LIMIT, Math.max(1, options.limit ?? DEFAULT_PAGINATION.limit))
...
export const buildOrgAccessCondition = <T extends AnyDrizzleTable>(
  ...
  const orgIdColumn = (table as Record<string, unknown>).orgId as Parameters<typeof eq>[0]
...
export const buildNestedOrgAccessCondition = <U extends AnyDrizzleTable>(
  ...
  const orgIdColumn = (joinTable as Record<string, unknown>).orgId as Parameters<typeof eq>[0]
...
export const combineConditions = (...conditions: (SQL | undefined)[]): SQL | undefined => {
...
export const createBaseQuery = <T extends AnyDrizzleTable>(ctx: ProtectedTRPCContext, table: T) =>
...
export const applyWhereClause = <Q extends DrizzleQuery>(query: Q, condition?: SQL): Q => {
  if (condition) {
...
export const applyOrderBy = <Q extends DrizzleQuery>(query: Q, orderClauses: SQL[]): Q => {
...
export const createOptimizedQuery = <T extends AnyDrizzleTable>(
  ...
  const sortClauses = createSortClause(table as unknown as Record<string, AnyColumn>, options.sort)
