...
export const createEntitySchema = <T extends z.ZodRawShape>(fields: T) => z.object(fields)
...
export const createBatchItemSchema = <T extends z.ZodRawShape>(itemSchema: T) =>
  z.object({
    items: z.array(z.object(itemSchema)).nonempty({ message: "At least one item is required." }),
  })
...
export const createUpdateSchema = <T extends z.ZodRawShape>(
  ...
  return partialSchema.extend(
    Object.fromEntries(requiredFields.map((field) => [field, baseSchema.shape[field]])),
  )
}
