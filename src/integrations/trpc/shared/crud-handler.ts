...
import type { Action, EntityType } from "#/db/schema"
...
export type CreateOptions = {
  getNextOrder?: (ctx: ProtectedTRPCContext, data: Record<string, unknown>) => Promise<number>
  beforeCreate?: (ctx: ProtectedTRPCContext, data: Record<string, unknown>) => Promise<void>
  afterCreate?: (ctx: ProtectedTRPCContext, entity: Record<string, unknown>) => Promise<void>
}
...
export type UpdateOptions = {
  ...
  afterUpdate?: (ctx: ProtectedTRPCContext, entity: Record<string, unknown>) => Promise<void>
}

export type DeleteOptions = {
  beforeDelete?: (ctx: ProtectedTRPCContext, id: number) => Promise<void>
  afterDelete?: (ctx: ProtectedTRPCContext, entity: Record<string, unknown>) => Promise<void>
}
...
export async function createEntity(
  ...
  const entityTitle =
    (entity.title as string) ?? (entity.name as string) ?? `${entityName} ${entityId}`
...
export async function updateEntity(
  ...
  let whereCondition = eq((table as unknown as { id: AnyTable["_"]["columns"]["id"] }).id, id)
  ...
  const result = await ctx.db.update(table).set(data).where(whereCondition).returning()
  const entity = Array.isArray(result) ? result[0] : result
...
export async function findEntityById(
  ...
  const result = await ctx.db.select().from(table).where(whereCondition).limit(1)
...
export async function findManyEntities(
  ...
  if (baseOptions.orgAccessCondition) {
    const orgCondition = baseOptions.orgAccessCondition(orgId)
    whereCondition = whereCondition ? and(whereCondition, orgCondition) : orgCondition
  } else if (baseOptions.nestedOrgAccessCondition) {
    const nestedCondition = baseOptions.nestedOrgAccessCondition(ctx)
    whereCondition = whereCondition ? and(whereCondition, nestedCondition) : nestedCondition
  }
...
export function createCrudHandlers(baseOptions: CrudOptions) {
  return {
    ...
    delete: (ctx: ProtectedTRPCContext, id: number, options: DeleteOptions = {}) =>
      deleteEntity(ctx, id, baseOptions, options),

    findById: (ctx: ProtectedTRPCContext, id: number) => findEntityById(ctx, id, baseOptions),

    findMany: (ctx: ProtectedTRPCContext, condition?: SQL, options: ListOptions = {}) =>
